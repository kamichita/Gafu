...
import json
import datetime
from datetime import timedelta  # 追加: timedeltaをインポート
...
intents.guilds = True
intents.members = True  # サーバーのメンバーに対する操作を許可
...
async def on_message(message):
    ...
    if message.content == "!help-command":
        help_text = (
            ...
            "8. `!ban <ユーザーID>` - ユーザーをバン\n"
            "9. `!kick <ユーザーID>` - ユーザーをキック\n"
            "10. `!timeout <ユーザーID> <時間（秒）>` - ユーザーにタイムアウトを与える"
        )
    ...
    if message.content.startswith("!ban"):
        if message.author.id in admin_ids:
            try:
                user_id_to_ban = int(message.content.split(" ")[1])
                member = message.guild.get_member(user_id_to_ban)
                if member:
                    await member.ban(reason="管理者によるバン")
                    await message.channel.send(f"✅ ユーザー {member.name} をバンしました。")
                else:
                    await message.channel.send("❌ ユーザーがサーバーに存在しません。")
            except (IndexError, ValueError):
                await message.channel.send("❌ コマンドの形式が正しくありません。使用方法: `!ban <ユーザーID>`")
        else:
            await message.channel.send("❌ あなたには管理者権限がありません。")
        return
    ...
    if message.content.startswith("!kick"):
        if message.author.id in admin_ids:
            try:
                user_id_to_kick = int(message.content.split(" ")[1])
                member = message.guild.get_member(user_id_to_kick)
                if member:
                    await member.kick(reason="管理者によるキック")
                    await message.channel.send(f"✅ ユーザー {member.name} をキックしました。")
                else:
                    await message.channel.send("❌ ユーザーがサーバーに存在しません。")
            except (IndexError, ValueError):
                await message.channel.send("❌ コマンドの形式が正しくありません。使用方法: `!kick <ユーザーID>`")
        else:
            await message.channel.send("❌ あなたには管理者権限がありません。")
        return
    ...
    if message.content.startswith("!timeout"):
        if message.author.id in admin_ids:
            try:
                user_id_to_timeout = int(message.content.split(" ")[1])
                timeout_duration = int(message.content.split(" ")[2])
                member = message.guild.get_member(user_id_to_timeout)
                if member:
                    await member.timeout(discord.utils.utcnow() + timedelta(seconds=timeout_duration), reason="管理者によるタイムアウト")
                    await message.channel.send(f"✅ ユーザー {member.name} に {timeout_duration} 秒のタイムアウトを与えました。")
                else:
                    await message.channel.send("❌ ユーザーがサーバーに存在しません。")
            except (IndexError, ValueError):
                await message.channel.send("❌ コマンドの形式が正しくありません。使用方法: `!timeout <ユーザーID> <時間（秒）>`")
        else:
            await message.channel.send("❌ あなたには管理者権限がありません。")
        return
